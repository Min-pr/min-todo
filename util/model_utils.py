...
if __name__ == '__main__':
    ...
    # dict 에 있는 키/값을 pyanmo db model에 설정하는 예제
    user = UserModel()
    signup_post_request = {
        'username': 'Test',
        'password': '1234',
    ...
    print('변경여부 : ', update_model_from_args(user, signup_post_request, excluded_keys=['email']))
    print('모델 데이터: ', user)

    # model 데이터를 list 로 변환후 개수를 확인한다.
    user_models = UserModel.scan()
    print('모델 데이터 개수 : ', get_total_count_pynamo_models(user_models))